...
[d61] 2026-09-16 · AMMR 운전 모드·최근 명령 재요청 계약 반영·정합 정비 112~118차 누적 (개인용·push)
- 본체 — SRS d407→d486 · SAD d597→d664 · 버전 v1.2.0→v1.3.0 (업체용 SRS 를 따름) (AMMR 운전 모드 자동·수동 축 신설[헤더 mode·전환 시 일괄 보고·수동 중 지시 거부·자율 동작 정지·배정 상위 조건·재시작 시 수동 시작] · 최근 명령 재요청 경로 신설[마지막 지시 재발행·상태 선행 보고·되살리지 않는 다섯 경우와 거절 회신] · HW 상태를 도킹·일시 정지·원점 복귀 포함 12종으로 확장 · 안전 정지·원점 복귀[접근 중 중단·담당자 복귀]·작업 취소 절 신설 · 장애 진입을 로봇 장애 보고 한 경우로 좁히고 해제를 담당자 조치 후 화면 해제로 일원화 · Gripper 파지 확인 실패 두 시점 장애 전이 · 잔존 Unit 되돌리기 목적지와 동적 재탐색 시 Move 재발행 규칙 확정)
- 외부전달 — ICD_AMMR d260→d355 · SRS_AMMR d212→d291 · UI정의제안 d172→d237 · 설비SlotID_AMMR d28→d38 · 버전 ICD·SRS_AMMR·UI정의제안 v1.2.0→v1.3.0 · 설비SlotID_AMMR v1.2.0→v1.2.1 (수행 미진입 거부 회신의 hw_state 는 회신 시점 현재 상태 · job_concurrent_request 도 같고 일시 정지 중이면 paused · ICD 에 A-10 요청·C-6 회신·C-7 거절 계약과 resume 계열 사유 6종·Gripper 2종·작업 취소 1종·동시 수신 거부 신설 · 사유 코드 개명 계열[자기 진단·주행 계통·저전력 상태형] 정리와 core_down 폐지 · 재충전 임계치 신설·충전 종료 임계치 개명·한도 단위 초 통일 · 태블릿에 [Reset]·[작업 취소]·[최근 명령 재요청] 버튼과 수동 조작 화면·원점 복귀 두 버튼·하단 안내 줄 신설 · 업체 대면 문서에서 Core 내부 판단 로직과 DB Table 이름 노출 제거 · 목록은 운영 대수 표현을 물류 AMMR 대수로 좁히고 공정 코드 정의 위치 포인터 추가)
- RnR — d78 무변경 (본문 갱신 없음 · 기존 발행 시각 유지)
