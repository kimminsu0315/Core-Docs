...
> 이 문서는 `Core_ICD_AMMR_v0_1_d79.md` 기준으로 작성되었습니다.
...
| C-5 설정 상태 조회 요청 (예비) | Core 요청 | 수신 확인 있음 — 별도 확인 메시지 없이 설정 상태 보고(A-9) 도착 자체가 확인. C-5는 예비 수단이다 (§4.2·§5.2 C-5) |
...
| A-9 | `ammr/{ammr_id}/state/config`     | 설정 상태 보고 (예비)   | 설정 상태 조회 요청(C-5) 수신 시(예비)        | 요청 수신 시점           |
...
| C-5 | `core/ammr/{ammr_id}/config/request` | 설정 상태 조회 요청 (예비) | Core가 현장 설정값을 확인해야 할 때 (예비·기본 경로 아님) | 발생 시점 |
...
> **예비 수단** — 설정 상태 조회 요청(C-5)에 대한 응답으로만 발행한다. 설정값의 단일 소스는 태블릿이며, Core는 이 값을 보유하지 않고 필요한 시점에만 조회한다.

- **Topic**: `ammr/{ammr_id}/state/config`
- **Trigger**: 설정 상태 조회 요청(C-5) 수신 직후 1회
...
- **목적**: AMMR에게 설정 상태 보고(A-9) 발행을 요청한다. AMMR은 이 메시지 수신 시 현재 태블릿 설정값을 보고한다.
- **수신 확인**: 별도 확인 메시지 없이 설정 상태 보고(A-9) 도착 자체가 수신 확인이다. 요청 후 3초(§7.3) 안에 도착하지 않으면 Core는 재요청할 수 있다.
...
| 32 | 설정 조회     | 설정값 단일 소스 = 태블릿 · Core는 보유하지 않고 설정 상태 조회 요청(C-5·예비)으로만 확인 · 응답 = 설정 상태 보고(A-9·6종·태블릿 설정 화면 순서) | §5.1 A-9, §5.2 C-5, §8.5 |
