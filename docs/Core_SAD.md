...
> 이 문서는 `Core_SAD_v1_0_d154.md` 기준으로 작성되었습니다.
...
- **Restack Process WIP Adapter** — CNC WIP 중계 프로그램으로부터 REST API로 입/출고 Event(되담기 완료 플래그 포함) 수신. State Service에 전달하고, REST 응답 본문에 Unit ID Mapping 정보(갱신된 Tray 형태 포함)를 실어 회신한다. 두 WIP Adapter 모두 응답 본문이 단순 확인 응답이 아니라 의미 있는 도메인 데이터를 실어 보내는 채널이다.
...
**작업대 슬롯 ID와 입고/출고.** CNC 작업대의 슬롯은 식별자 명명 규칙의 `CNC-{장비명}-{BEFORE|AFTER}` 형식으로 식별된다 (BEFORE=입고·AFTER=출고, SM MACHINE_NAME·입출고 신호 기반). 입고/출고 판정은 State Service가 SM Slot Sensor 신호를 해석해 이벤트에 쓰며, Core는 작업대별 슬롯 ID를 InMemory에 보유한다. 물류 AMMR Job 지시 시 목적지·출발지 슬롯은 이 슬롯 ID로 지정한다 — AMMR은 역할이 아니라 물리 위치로 이동하기 때문이다.
...
```
CNC WIP 중계 프로그램 (Slot Sensor On/Off + QR Scan[입/출고 구분값] + 되담기 완료 플래그·적층 구성 리스트)
