...
> 이 문서는 `Core_SAD_v1_0_d147.md` 기준으로 작성되었습니다.
...
**작업대 물리 슬롯 라벨과 입고/출고 매핑.** CNC 작업대의 슬롯은 물리적으로 A1·A2 등 슬롯 라벨로 식별된다. 입고/출고는 State Service가 SM Slot Sensor 신호를 해석해 입출고 판단·이벤트에 쓰는 역할 개념이며, Core는 작업대별로 입고/출고 역할을 물리 슬롯 라벨에 매핑해 InMemory에 보유한다. 물류 AMMR Job 지시 시 목적지·출발지 슬롯은 이 물리 라벨(A1·A2 등)로 지정한다 — AMMR은 역할이 아니라 물리 위치로 이동하기 때문이다.
...
```
공정 WIP 중계 프로그램 (Slot Sensor On/Off + QR Scan[입/출고 구분값])
...
State Service (InMemory 갱신 → 정합성 검사 → Event 발행)
    ...
    ↓ ├─ 출고 QR 인식 Event ─→ State Service 자체 (Unit 출고 시간 기록)
    ↓ │                          └─→ Transfer Service (Transfer 생성)
...
| 출고 QR 인식 | Slot Unit 식별값 갱신(확정 등급) + Unit 출고 시간 기록 | 출고 QR 인식 Event |
...
| 출고 QR 인식 Event | State Service 자체 | Unit 출고 시간 기록 |
| 출고 QR 인식 Event | Transfer Service (Push) | Transfer 생성(멱등), 우선순위 정렬 + State Service 경유 Transfer 이력 DB 기록 (멱등 skip 시에도 skip 플래그 True 레코드로 기록) |
...
출고 QR 인식 Event가 Transfer Service에 도달했으나 동일 Unit의 미수행 Transfer가 TransferList에 이미 존재하여 신규 생성이 skip된 경우, Transfer 이력 DB에 skip 플래그(True) 레코드를 정상 Transfer와 동일 경로(State Service 경유)로 기록한다. 레코드는 정상 Transfer와 동일한 유니크 ID 체계를 가지되 Job 이력 매칭 대상이 아니다(매칭 규칙은 skip 플래그 False 레코드에 한정). 멱등 skip은 정상 흐름에서 발생하지 않는 이상 신호(중계 프로그램 재전송·네트워크 재시도·운영자 중복 Scan 등)이므로 사후 원인 추적을 위해 기록 공백을 두지 않는다. Dashboard 경고 연동(임계치·경고 방식)은 코드 레벨 결정 영역(운영 후 skip 플래그 True 레코드 빈도 지표 기반).

### 5.4 되담기 WIP 입/출고 → 되담기 처리 → 상태 갱신 → Transfer 생성

```
되담기 WIP 중계 프로그램 (Slot Sensor On/Off + QR Scan[입/출고 구분값] + 되담기 완료 플래그·적층 구성 리스트)
...
State Service (InMemory 갱신 → 정합성 검사 → Event 발행)
    │
    ↓ ┌─ 입고 QR 인식 Event ─→ State Service 자체 (Unit 입고 시간 기록)
    ↓ ├─ 출고 QR 인식 Event ─→ State Service 자체 (Unit 출고 시간 기록 + Tray 형태 갱신: 적층 Tray + 적층 Unit 구성 매핑)
    ↓ │                          └─→ Transfer Service (Transfer 생성)
    ↓ ├─ Slot Sensor On Event ─────────→ (Transfer Service 소비 없음)
    ↓ ├─ Slot Sensor Off Event ─────────→ Transfer Service (자기 대기 Transfer의 출발 Slot이면 무효화)
    ↓ └─ Slot Sensor Off Event ─────────→ AMMR Service (만재 대기 AMMR 깨우기)
...
| 출고 QR 재Scan + 되담기 완료 플래그 → Core Push | 되담기 WIP 중계 프로그램 | REST 호출, 되담기 완료 여부 + 적층 구성 리스트(어느 Tray에 무엇을 담았는지) 함께 전달 |
...
| 입고 QR 인식 | Slot Unit 식별값 갱신(확정 등급) + Unit 입고 시간 기록 | 입고 QR 인식 Event |
| 출고 QR 인식 (+ 되담기 완료 플래그·구성 리스트) | Slot Unit 식별값 갱신(확정 등급) + Unit 출고 시간 기록 + Tray 형태 갱신 (공정 Tray → 적층 Tray) + 적층 Tray Unit 구성 매핑(구성 리스트 기준) | 출고 QR 인식 Event |
...
**되담기 공정 Recipe 검증.** 되담기 WIP 통합 Slot에서 입고 QR 인식이 들어오면 State Service는 Unit Recipe에 'Tray 되담기' 공정이 명시되어 있는지 동반 검증한다. 명시되지 않은 Unit이 입고된 경우 정합성 이상으로 해당 Slot에 Block을 마킹한다(RestackRecipeMismatch). Core는 외부에서 본 것만 기록하므로, Dashboard에서 Unit Recipe를 사후 갱신하더라도 InMemory Recipe만 바뀔 뿐 Block 해제로는 작동하지 않는다. Block 해제 경로는 두 가지다 — 사람이 Slot에서 Unit을 회수해 Sensor Off로 자연스럽게 정합되거나, QR 재Scan으로 Slot Unit 식별값이 다시 들어와 동일 검증이 재작동하여 정합성 회복되는 경우. 이 검증은 Slot Sensor 식별값과 Slot Unit 식별값의 정합성을 보는 일반 검사와 별도로, Unit Recipe 차원에서 작동한다.

**적층 Tray Unit 구성 매핑.** 적층 Tray는 여러 단을 쌓은 스택이며, 되담기 과정에서 CNC 작업 등으로 쌓이는 순서가 뒤집힐 수 있다(예: 5·4·3·2·1 → 1·2·3·4·5). 각인 QR은 스택 최상단 Tray만 인식되므로 최상단 QR 하나로는 스택 전체 구성을 알 수 없다. 되담기 WIP 중계 프로그램이 되담기를 완료하면 출고 QR 재Scan·되담기 완료 플래그와 함께 어느 Tray에 무엇을 담았는지 구성 리스트를 REST로 전달하고, State Service는 출고 QR 인식 atomic 안에서 이 리스트로 적층 Tray Unit 구성을 완성해 매핑한다(Tray 형태를 적층 Tray로 갱신하며 함께 수행). Unit 정의 자체는 불변이며, 구성 완성만 되담기 완료 시점에 확정된다. 짝 공정 Tray와의 매칭·되담기 수행은 되담기 WIP 중계 프로그램 측 책임이다(Core 미관여).
...
|---|---|---|
| 입고 QR 인식 Event | State Service 자체 | Unit 입고 시간 기록. Transfer Trigger 아님 |
| 출고 QR 인식 Event | State Service 자체 | Unit 출고 시간 기록 + Tray 형태 갱신 + 적층 Tray Unit 구성 매핑 |
| 출고 QR 인식 Event | Transfer Service (Push) | Transfer 생성(멱등, 적층 Tray 전환 완료 시 다음 공정으로), 우선순위 정렬 + State Service 경유 Transfer 이력 DB 기록 (멱등 skip 시에도 skip 플래그 True 레코드로 기록) |
| WIP Slot Sensor Off Event | Transfer Service (Push) | 자기 대기 Transfer의 출발 Slot이 해제됐으면 자동 무효화 |
| WIP Slot Sensor Off Event | AMMR Service (Push) | 만재 대기 AMMR 깨우기 — 관리 AMMR 중 6 Slot 만재 대기 상태가 있을 때만 만재 Unit Dropoff 재개 판정 |
...
Charge는 AMMR Service가 자체 생성하는 단일 Job이며 Transfer 미경유다. Core는 충전 스테이션을 지정하지 않고 단일 Charge Job(위치 미포함)으로 지시하며, AMMR은 태블릿 설정값으로 보유한 충전 스테이션으로 자체 이동·도킹한다. 이 설정 스테이션은 Core 지시 충전과 AMMR 자율 충전(저전력) 모두에 쓰인다. 충전존 이동·도킹·충전·이탈 등 물리적 수행 세부와 충전 중단 결정(자체 임계)은 AMMR HW가 자체 처리한다 — Core는 도킹 시점에 발행되는 Job 수행 결과 통합 보고를 수신하여 '충전 중' 전이를 인지하고, '충전 중→대기' 전이(자체 임계 + 충전 중단 시점)는 AMMR 상태 변화 Event로 별도 보고된다(권위 위치는 이 절 권위 분담 표).
...
| AMMR HW 상태 갱신 (Move/Pickup/Dropoff/Charge/대기/충전 중/저전력/장애) | 초기 연결 시 일괄 보고 또는 AMMR HW 자체 전이 Event | AMMR HW 상태 | 상태 전이 이력 기록 | AMMR 상태 변화 Event |
...
**※ AMMR 태블릿 표시 응답.** State Service는 AMMR 보고 처리 [수신] atomic 안에서 태블릿 표시 정보를 MQTT 발신 게이트웨이로 해당 AMMR에 회신한다 — AMMR Slot Sensor 변동·Job 수행 결과 처리 시 해당 Slot의 해석 정보(Slot Unit 식별값·Unit 제품 정보·Job 배정·시스템 상태[정합성 판정]), AMMR HW 상태 전이 처리 시 Core가 판단한 AMMR 상태·작업 Slot, 일괄 보고 처리 시(초기 연결·재연결·일괄 보고 재요청 응답 포함) 6 Slot 전체의 해석 정보 일괄. 모든 회신에 Core가 판단한 AMMR 상태·작업 Slot이 함께 실린다. AMMR에 Mount된 태블릿이 이 응답으로 적재 상태·배정 상태·상단 정보를 표시한다. 이 회신은 SignalR Dashboard Push와 함께 atomic 안에서 수행된다.
...
|---|---|---|---|
| WIP Slot | WIP 중계 프로그램 Sensor 신호 | WIP 중계 프로그램 QR 인식 (입/출고) + Dashboard 수동 입력 | AMMR Pickup·Dropoff 동작 보고 |
...
**AMMR Slot 적재 정보 일괄 재로드.** AMMR 태블릿의 일괄 재로드는 6개 Slot 전체의 Sensor 상태와 담당자 입력 식별값을 MQTT 경유 일괄 입력으로 State Service에 전달한다. State Service는 Slot별로 Slot Sensor 식별값·Slot Unit 식별값(확정 등급)을 갱신하고 정합성 검사를 적용하며(단일 Slot 수동 입력 처리와 동일 경로), 정합이 회복된 Slot은 Block이 자동 해제되고 확정 등급으로 잔존 Unit이 드러난 AMMR Slot은 잔존 Unit Transfer 처리로 이어진다. MQTT 응답에는 Slot별 결과(정합·빈 Slot·정합성 이상)와 해당 Unit 정보·Job 배정을 실어 회신한다. 재로드 정합성 검사는 Sensor 식별값과 Unit 식별값 정합 외에 입력 Unit의 Recipe·경로 유효성도 포함한다 — Unit이 갈 수 없는 공정이나 수행할 수 없는 작업으로 판정되면 정합성 이상(Block)으로 응답한다. 이 Recipe·경로 유효성 위반은 정합성 이상 Block의 일반 사유이며, 되담기 공정 미명시 검증(RestackRecipeMismatch)이 그 한 사례다.
...
동적 Fallback 대체 경로는 Job 성격(이동 출발지 종류)에 따라 분기한다. **일반 이동** = WIP → WIP 이동, Pair 절차 무관 → 출발 Slot 복귀로 단순 처리. **CNC 관련 이동** = CNC 작업대에서 다음 목적지(WIP)로 가는 이동, 가공 결과 Unit이라 Dropoff 실패 시 1순위로 되담기 WIP **통합 Slot**(되담기 완료 Unit이 놓이는 빈 자리)에 임시 보관(되담기 작업 메커니즘 자체와 무관, 통합 Slot이 임시 보관 위치로 활용됨). 2순위는 출발 Slot 복귀.
...
|---|---|---|
| 일반 이동 | 출발 Slot 복귀 | — |
| CNC 관련 이동 | 되담기 WIP Slot | 출발 Slot 복귀 |
...
| General Process WIP Adapter · Restack Process WIP Adapter | WIP 중계 프로그램이 REST 응답 수신 재개 시점에 Slot Sensor 식별값 + Slot Unit 식별값(QR 인식)을 일괄 재전송. HealthCheck 수신 재개만으로는 Block 해제되지 않음. |
| MQTT Adapter | AMMR 측 재연결(AMMR HW 단절 복구·Broker 재시작) 시는 연결 시 일괄 보고(AMMR HW 상태·6 Slot 점유 Sensor·위치·Battery)로, Core 측만 재연결(MQTT Adapter 단독 재연결·Core 재시작) 시는 State Service 발신 게이트웨이 경유 일괄 보고 재요청 발신 → AMMR 재발행 수신으로 재동기화. 정합성 검사 자동 작동 — Sensor On + Unit 식별값 Null은 Block 마킹 → Dashboard Unit 식별값 수동 입력으로 해소, Sensor Off는 자연스럽게 정합. Core 논리 AMMR 상태 Null은 재연결 후 정상 운영 모드 복귀 시 다음 상태 갱신. |
