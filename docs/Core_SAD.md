...
> 이 문서는 `Core_SAD_v1_0_d152.md` 기준으로 작성되었습니다.
...
※ 이 코드블록은 AMMR 1대 단위 적용 (Last Will은 AMMR HW별 등록). MQTT Adapter 두절(Broker 연결 끊김) 시는 모든 AMMR 일괄 적용. 재연결 시 일괄 보고가 Slot 점유와 Unit 식별값을 함께 실어 정합성 검사·재구축 자동 작동.
